...
try:
    ...
    with open(path, newline='', encoding='utf-8') as input_file:
        ...
        with open('output_{name}'.format(name=os.path.basename(input_file.name)), 'w', newline='', encoding='utf-8') \
                as output_file:
            ...
            for row in reader:
                ...
                logging.info((API_ENDPOINT.format(key=key, address=quote('+'.join(row)))))
                if len(row):
